"""
Testing train_resnet code on MNIST dataset
...
training_dataset = MNIST(mnist_data_path, train=True, download=True, transform=base_transform)
training_generator = DataLoader(training_dataset, batch_size=train_batch_size, shuffle=True)
test_dataset = MNIST(mnist_data_path, train=False, download=True, transform=base_transform)
test_generator = DataLoader(test_dataset, batch_size=test_batch_size, shuffle=True)

train_dom_count = np.zeros(10, dtype='int')
for i in range(len(training_dataset)):
    train_dom_count[training_dataset[i][1]] += 1
test_dom_count = np.zeros(10, dtype='int')
for i in range(len(test_dataset)):
    test_dom_count[test_dataset[i][1]] += 1
print('Number of each label in training set: {}/{}/{}/{}/{}/{}/{}/{}/{}/{}'
      .format(train_dom_count[0], train_dom_count[1], train_dom_count[2], train_dom_count[3], train_dom_count[4],
              train_dom_count[5], train_dom_count[6], train_dom_count[7], train_dom_count[8], train_dom_count[9]))
print('Number of each label in testing set: {}/{}/{}/{}/{}/{}/{}/{}/{}/{}'
      .format(test_dom_count[0], test_dom_count[1], test_dom_count[2], test_dom_count[3], test_dom_count[4],
              test_dom_count[5], test_dom_count[6], test_dom_count[7], test_dom_count[8], test_dom_count[9]))
...
resnet18_base.conv1 = torch.nn.Conv2d(1, 64, kernel_size=7, stride=2, padding=3, bias=False)
